...
- **Product name** — must match the `productName` your extension passes
  to the SDK. Shared across tiers, and part of the verification contract
  baked into shipped builds: it can't be edited later.
...
export const client = createActivationClient({
  productName: 'My Extension',            // = the plan's product name
  plans: {
